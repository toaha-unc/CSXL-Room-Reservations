...
class ReservationService:
    ...
    def draft_reservation(
        ...
        """
        Updated method to handle reservations for multiple users.
        ...
        # Enforce Reservation Draft Permissions for multiple users
        if subject.id != request.users[0].id:
            self._permission_svc.enforce(
                subject, "coworking.reservation.manage", f"user/{request.users[0].id}"
            )

        # Bound start and end times as before
        now = datetime.now()
        start = max(request.start, now)

        # Determine if the reservation is a walk-in
        ...
        end = min(request.end, end_limit)

        bounds = TimeRange(start=start, end=end)

        # if seat is empty reserve seats automatically for users
        if len(request.seats) == 0:
            # Fetch all seats
            all_seats = self._seat_svc.list()
            # Get availability for all seats in the defined bounds
            seat_availability = self.seat_availability(all_seats, bounds)
            # Filter out non-reservable seats if it's not a walk-in
            if not is_walkin:
                seat_availability = [
                    seat for seat in seat_availability if seat.reservable
                ]
            # Check if there are enough available seats for all users
            if len(seat_availability) < len(request.users):
                raise ReservationException("Not enough seats available for all users.")
            # Select a number of seats equal to the number of users
            # Map SeatAvailability to SeatIdentity (assuming .seat gives us SeatIdentity or the required identifier)
            request.seats = [
                seat_avail for seat_avail in seat_availability[: len(request.users)]
            ]
        ...
        # Check and assign seats for all users
        ...
        # Assign seats to users, respecting the bounds of seat availability
        seat_entities = [
            self._session.get(SeatEntity, seat.id)
            for seat in seat_availability[: len(request.users)]
        ]
        bounds = seat_availability[0].availability[0]

        # Create the draft reservation for multiple users
    ...
    def check_users_in_reservations(self, subject: User, pids: Sequence[int]) -> str:
        """Check if any user in the list is part of active reservations (not cancelled or checked out).

        Args:
            pids (Sequence[int]): List of user pids.
            subject: a valid user 

        Returns:
            str: A message indicating whether any users are in an active reservation, listing their pids.
        """
        users_in_reservations = []
        ...
        for pid in pids:
            reservations = (
                ...
                .join(ReservationEntity.users)
                .filter(
                    UserEntity.pid == pid, ReservationEntity.state.in_(active_states)
                )
                .all()
            )
            if reservations:
                users_in_reservations.append(str(pid))

        if users_in_reservations:
            pids_str = ", ".join(users_in_reservations)
            return f"User(s) with PID(s) {pids_str} are in a reservation"

        return "No users in a reservation"