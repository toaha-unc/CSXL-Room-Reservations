...
export class CoworkingDropInCard implements OnChanges {
  ...
  public groupCategories: SeatCategory[];
  router: any;

  constructor() {
    this.categories = this.initCategories();
    this.groupCategories = this.initGroupCategories();
  }
    ...
    for (let seat of this.seat_availability) {
      if (seat.has_monitor) {
        if (seat.sit_stand) {
          ...
          this.groupCategories[SITTING_BENCH].push(seat);
        }
      } else {
        this.categories[COLLAB_AREA].push(seat);
        this.groupCategories[COLLAB_AREA].push(seat);
      }
    }
  }
  ...
  private initGroupCategories(): SeatCategory[] {
    return [new SeatCategory('Communal Area Seat')];
  }
